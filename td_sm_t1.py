...
# =============================================================================
# GRASSHOPPER CONSTANTS
# sprite: 240×36, 4 frames → each frame is 60×36
# =============================================================================
GRASSHOPPER_FRAME_W     = 60    # 240 / 4 frames
GRASSHOPPER_FRAME_H     = 36
GRASSHOPPER_FRAME_COUNT = 4
GRASSHOPPER_FRAME_DELAY = 18    # slow idle animation
GRASSHOPPER_SPEED       = 0.6   # very slow wander
GRASSHOPPER_WANDER_RAD  = 70    # max pixels from lily pad centre it can roam
GRASSHOPPER_HITBOX      = 22    # pixel radius for tongue collision check
GRASSHOPPER_POINTS      = 15    # hunger points gained when eaten
GRASSHOPPER_MAX         = 2     # maximum on screen at once
GRASSHOPPER_RESPAWN     = 420   # frames before a new one spawns (7 seconds)

# =============================================================================
# FROGLET EAT (TONGUE) CONSTANTS
# sprite: 1792×128, 7 frames → each frame is 256×128
# The sprite faces LEFT in the file. We flip it to face RIGHT on load.
# The froglet body occupies the right half; the tongue extends to the left.
# After flipping: body on LEFT, tongue extends to the RIGHT.
# Centering the sprite on the player head puts the tongue in the facing direction.
# =============================================================================
FROGLET_EAT_FRAME_W     = 256   # 1792 / 7 frames
FROGLET_EAT_FRAME_H     = 128
FROGLET_EAT_FRAME_COUNT = 7
FROGLET_EAT_FRAME_DELAY = 5     # ticks per frame (full anim ≈ 35 ticks ≈ 0.58s)
TONGUE_MAX_LENGTH        = 200   # world pixels the tongue tip reaches at full extend

# Fraction of tongue extension at each frame index 0–6.
# Peaks at frame 3, then retracts. Used to position the tongue tip for hit detection.
...
# Grasshopper sprites — both directions loaded
...
# Froglet eat sprite faces LEFT in the file.
# We flip it horizontally so it faces RIGHT, matching our rotation convention.
# After flip: body on the RIGHT half, tongue extends from center outward to RIGHT.
# Centering this sprite on the player head then rotating by angle_deg puts the
# tongue in exactly the direction the froglet is facing.
...
# =============================================================================
# BLOB CENTRE FINDER (shared logic for crabs and grasshoppers)
# Scans a surface for pixels of a given colour, groups them into blobs,
# and returns the centroid of each blob.
# skip_r  = minimum distance between two different blob centroids
# scan_step = how many pixels to skip between checks (higher = faster)
# =============================================================================
...
del crabzone_surf, gh_spawn_surf   # no longer needed — free memory
...
def is_in_gh_zone(x, y):
    # checks if a pixel is inside any lily pad green zone
    ix, iy = int(x), int(y)
    if 0 <= ix < WORLD_W and 0 <= iy < WORLD_H:
...
# =============================================================================
# GRASSHOPPER
# Sits on one lily pad (green zone blob). Wanders slowly within that zone.
# Can only be eaten by the froglet's tongue (E key).
# =============================================================================
class Grasshopper:
    def __init__(self, origin_x, origin_y):
        ...
        self.angle       = random.uniform(0, 2 * math.pi)
        self.facing_right= True            # determines which sprite to show
    ...
    def update(self):
        ...
        if self.wander_timer <= 0:
            # pick a new wander direction every so often
            self.angle       = random.uniform(0, 2 * math.pi)
            self.wander_timer= random.randint(60, 200)
        ...
        # Try to move within the green zone AND within wander radius of origin
        ...
        if is_in_gh_zone(nx, ny) and dist_from_origin <= GRASSHOPPER_WANDER_RAD:
            ...
            # Hit the zone edge or wander limit — pick a new direction
            self.angle = random.uniform(0, 2 * math.pi)
    ...
    def draw(self, surface, cam):
        ...
        # Draw centered — grasshoppers don't use head-pivot rotation
...
class Player:
    def __init__(self):
        ...
        # Tongue / eat (froglet only)
        # The full 7-frame animation must complete before another eat is allowed.
        self.eating        = False
        self.eat_frame     = 0
        self.eat_anim_timer= 0
        self.tongue_hit_done = False   # prevents eating multiple grasshoppers per tongue flick
    ...
    def evolve_to_toad(self):
        ...
        self.eating = False   # cancel any tongue animation on evolve
    ...
    def try_boost(self):
        # Boost only available when froglet and not already eating or boosting
    ...
    def try_eat(self):
        # Tongue only usable when froglet, not boosting, and not already eating
        if self.stage == "froglet" and not self.boosting and not self.eating:
            self.eating         = True
            self.eat_frame      = 0
            self.eat_anim_timer = 0
            self.tongue_hit_done= False   # allow one hit this tongue flick

    def tongue_tip_world(self):
        """Returns the world-space position of the tongue tip for the current eat frame."""
        frac   = TONGUE_EXTENSION[min(self.eat_frame, FROGLET_EAT_FRAME_COUNT - 1)]
        length = TONGUE_MAX_LENGTH * frac
        rad    = math.radians(-self.angle_deg)   # convert pygame angle → math angle
    ...
    def update(self, keys):
        ...
        # ---- EAT ANIMATION TICK ----
        # Movement still works while eating — only the sprite changes.
        if self.eating:
            self.eat_anim_timer += 1
            if self.eat_anim_timer >= FROGLET_EAT_FRAME_DELAY:
                self.eat_anim_timer = 0
                self.eat_frame += 1
                if self.eat_frame >= FROGLET_EAT_FRAME_COUNT:
                    # Animation finished — ready to eat again
                    self.eating        = False
                    self.eat_frame     = 0
        ...
        # ---- NORMAL ANIMATION (not used when eating — sprite swaps) ----
    ...
    def check_tongue_hits(self, grasshopper_list):
        """
        Called every frame while eating.
        ...
        if not self.eating or self.tongue_hit_done:
            return
        frac = TONGUE_EXTENSION[min(self.eat_frame, FROGLET_EAT_FRAME_COUNT - 1)]
        if frac < 0.3:   # tongue barely out — skip collision check
            return
        ...
        for gh in grasshopper_list:
            if gh.alive:
                ...
                if dist < GRASSHOPPER_HITBOX:
                    gh.alive         = False
                    self.tongue_hit_done = True   # can't eat another this flick
                    self.eat(GRASSHOPPER_POINTS)
                    break   # one grasshopper per tongue flick
    ...
    def draw(self, surface, cam):
        ...
        if self.boosting:
            ...
            # ---- EAT SPRITE ----
            # The eat sprite (flipped to right-facing) has:
            #   - froglet body on the LEFT half of the 256px frame
            #   - tongue extending from center to RIGHT
            # By centering on the player head position and rotating,
            # the tongue naturally points in the direction the froglet faces.
            frame = froglet_eat_frames[min(self.eat_frame, FROGLET_EAT_FRAME_COUNT - 1)]
            if self.hit_timer > 0:
                frame = frame.copy()
                frame.fill((255, 80, 80, 120), special_flags=pygame.BLEND_RGBA_MULT)
...
# =============================================================================
# CRAB
# Sideways-walking, confined to its orange zone blob.
# =============================================================================
class Crab:
...
def make_grasshoppers():
    ...
    hoppers = []
    for ox, oy in chosen:
        # Find an actual green pixel near the blob centre to start on
        placed = False
...
# Grasshopper respawn timer — ticks up and respawns when needed
gh_respawn_timer = 0
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_e:
                # E key triggers tongue — only works as froglet
    ...
    if player.health > 0 and not player.won:
        ...
        # Check tongue against grasshoppers every frame while eating
        if player.eating:
        ...
        # Update larvae — eaten automatically by tadpole/toad on contact
        # (froglet eats larvae too but grasshoppers are its main food via E key)
        ...
        # Grasshopper respawn — tick timer, respawn on an unoccupied lily pad
        # if fewer than max are alive
        ...
        if gh_alive < GRASSHOPPER_MAX:
            ...
            if gh_respawn_timer >= GRASSHOPPER_RESPAWN and lily_pad_positions:
                gh_respawn_timer = 0
                # Find lily pads not currently occupied by a living grasshopper
                ...
                if available:
                    ...
                    # Reuse a dead grasshopper slot
                    for gh in grasshopper_list:
                        if not gh.alive:
                            gh.__init__(nx, ny)
                            break
                    # If all slots full (shouldn't happen) append a new one
                    else:
    ...
    # Draw in order: larvae (background), grasshoppers (on lily pads),
    # fish, crabs, player (foreground)
    for larva in larvae_list:
